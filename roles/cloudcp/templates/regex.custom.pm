...
if (($globlogs{CUSTOM1_LOG}{$lgfile}) and ($line =~ /(\S+).*] "\w*(?:GET|POST) \/xmlrpc\.php.*" /)) {
    return ("your ban comment",$1,"XMLRPCAttacks","5","80,443,21,25,22,23","3600");
    }
    
 # WP-LOGINS
if (($globlogs{CUSTOM1_LOG}{$lgfile}) and ($line =~ /(\S+).*] "\w*(?:GET|POST) \/wp-login\.php\s+HTTP\/\d\.\d"\s+200/)) {
    return ("your ban comment",$1,"WPLOGINAttacks","5","80,443,21,25,22,23","3600");
    }

# JOOMLA-LOGINS
if (($globlogs{CUSTOM1_LOG}{$lgfile}) and ($line =~ /(\S+).*] "\w*(?:GET|POST) \/administrator\/index\.php\b" /)) {
    return ("your ban comment",$1,"JOOMLAAttacks","5","80,443,21,25,22,23","3600");
    }
